...
export default function LongtimeSwiperCard({ route }) {
  ...
  async function fetchdata(paras1, paras2) {
    ...
    console.log(body);
    try {
      await fetch("http://192.168.1.8:5000/api/l_like_job", {
  ...
  async function setapplied(paras1, paras2) {
    ...
    console.log(body);
    try {
      await fetch("http://192.168.1.8:5000/api/longtime_apply_job", {
        method: "post", // *GET, POST, PUT, DELETE, etc.
        mode: "cors", // no-cors, *cors, same-origin
        cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
        credentials: "same-origin", // include, *same-origin, omit
  ...
  const getJobs = async () => {
    try {
      await fetch(
        `http://192.168.1.8:5000/api/count_apply_job/${route.params.userID}`,
        {
          method: "GET",
          mode: "cors",
          cache: "no-cache",
  ...
  const getdata = async () => {
    ...
    try {
      await fetch(
        `http://192.168.1.8:5000/api/limit/L_like_apply_check/${userID}`,
        {
          method: "POST",
          mode: "cors",
          cache: "no-cache",
